<template>
    <div class="box-border p-6 container mx-auto px-6">
        <form action="#" method="POST" @submit.prevent="createGroup">
            <div class="mx-auto py-5 bg-white space-y-6 sm:p-6">
                <div class="col-span-3 sm:col-span-2">
                    <label for="group_name" class="block text-sm font-medium text-gray-700">
                        Название группы
                    </label>
                    <div class="mt-1 flex rounded-md shadow-sm">
                        <input v-model="group_name" type="text" id="group_name"
                               class="focus:ring-pink-500 focus:border-pink-500 flex-1 block w-full rounded-none rounded-r-md sm:text-sm border-gray-300"
                               placeholder="Введите название">
                    </div>
                </div>

                <div class="col-span-3 sm:col-span-2">
                    <label for="course_id" class="block text-sm font-medium text-gray-700">
                        Направление
                    </label>
                    <SingleSelect v-model="course_id" @selected="updateCourse" :options="courses"
                                  id="course_id" :placeholder="'Выберите направление'"/>
                </div>

                <div class="col-span-3 sm:col-span-2">
                    <label for="course_id" class="block text-sm font-medium text-gray-700">
                        Направление
                    </label>
                    <SingleSelect v-model="specialization_id" @selected="updateSpecialization" :options="specializations"
                                  id="specialization_id" :placeholder="'Выберите специализацию'"/>
                </div>

            </div>


            <div class="px-4 py-3 bg-gray-50 text-right sm:px-6">
                <button @click="$parent.$emit('close');"
                        class="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                    Отменить
                </button>
                <button type="submit"
                        class="inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                    Сохранить
                </button>
            </div>
        </form>
    </div>
</template>

<script>

import SingleSelect from "@/Сomponents/SingleSelect";
export default {
    name: "GroupForm",
    components: {
        SingleSelect,
    },
    data() {
        return {
            group_name: '',
            course_id: null,
            specialization_id: null,
            specializations: [],
            courses: [],
        }
    },
    created() {
        this.coursesList()
    },
    methods: {

        coursesList()
        {
          axios.get('/api/courses/dictionary').then((res) => {
              if (res.status === 200)
              {
                  this.courses = res.data.data;
                  console.log(this.courses)
              }
          })
        },

        specializationsList(e)
        {
            axios.get('/api/specializations/dictionary/' + e).then((res) => {
                if (res.status === 200)
                {
                    this.specializations = res.data.data;
                    console.log(this.specializations)
                }
            })
        },
        createGroup()
        {
            console.log(this.course_id)
            axios.post('/api/groups', {
                name: this.group_name,
                specialization_id: this.specialization_id,
            }).then((res) => {
                if (res.status === 201)
                {
                    this.$parent.$emit('close');
                }
            }).catch((res) => console.log(res))
        },
        updateCourse(e) {
            this.course_id = e;
            this.specializationsList(e);
        },
        updateSpecialization(e) {
            this.specialization_id = e;
        }
    }
}

</script>

<style scoped>

</style>
